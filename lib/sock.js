...
module.exports = async function connectToWhatsApp(config, client) {
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (reason === DisconnectReason.badSession) {
          ...
          connectToWhatsApp(config, client);
          //sock.logout();
        ...
        const userName = sock.user.name || 'Undefined';
        const userId = sock.user.id.split(':')[0] || 'Undefined';
        
        console.log(chalk.bold(chalk.cyan.blue('• User Info')));
        console.log(chalk.cyan(`- Name: ${userName}`));
        console.log(chalk.cyan(`- Number: ${userId}`));
    ...
    sock.sendContacts = async (jid, contacts, quoted = '', opts = {}) => {
      let list = [];
      for (let i = 0; i < contacts.name.length; i++) {
        list.push({
          displayName: contacts.name[i],
          vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${contacts.name[i]}\nFN:${contacts.name[i]}\nitem1.TEL;waid=${contacts.number[i]}:${contacts.number[i]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
        });
      }
